 ...
 *
 * Copyright (C) 2006-2015 Curtis J. Blank curt@curtronics.com
 *
...
static long GetCEdata(int fdser, int yAddress, int opcode, int param, char description[]);
...
int GetVer(int fdser, int yAddress)
{
    ...
    if (nCnt > 0 && TransState((int)szSerBuffer[aState],_opGetVer,TRUE)) {
        ...
        //CHANGE -KEVIN
        //I don't want this information when I ask for version in Json format.
        //TODO: make these values available for JSON
        if (!bJSON){
...
int GetConf(int fdser, int yAddress)
{
    ...
    nCnt = Communicate(fdser, yAddress);
    crcValue = crc16(szSerBuffer, 8);
    if (nCnt > 0 && TransState((int)szSerBuffer[aState],_opGetConfig,TRUE) && !bJSON) {
...
int GetVerFW(int fdser, int yAddress)
{
    ...
    if (nCnt > 0 && TransState((int)szSerBuffer[aState],_opGetVerFW,TRUE)) {
        ...
        //CHANGE -KEVIN
        if (bJSON){
            fprintf(outfp, "\n\"Firmware\": \"%c.%c.%c.%c\",", szSerBuffer[aRel3],szSerBuffer[aRel2],szSerBuffer[aRel1],szSerBuffer[aRel0]);
        }else{
            fprintf(outfp, "\nFirmware: %c.%c.%c.%c\n",szSerBuffer[aRel3],szSerBuffer[aRel2],szSerBuffer[aRel1],szSerBuffer[aRel0]);
        }
        //END CHANGE -KEVIN
...
int GetCE(int fdser, int yAddress)
{
    ...
    //START CHANGE KEVIN
	  ...
	  fprintf(outfp, "\"cumulatedenergy\": {");
          fprintf(outfp, "\n\"%s\": %12.3f,",_CEDaily,DAILY/1000.0);
          fprintf(outfp, "\n\"%s\": %12.3f,",_CEWeekly,WEEKLY/1000.0);
          fprintf(outfp, "\n\"%s\": %12.3f,",_CEMonthly,MONTHLY/1000.0);
          fprintf(outfp, "\n\"%s\": %12.3f,",_CEYearly,YEARLY/1000.0);
          fprintf(outfp, "\n\"%s\": %12.3f",_CETotal,TOTAL/1000.0);
          fprintf(outfp, "\n}");
    }else{
    //END CHANGE KEVIN
...
/*--------------------------------------------------------------------------
    GetCEdata
----------------------------------------------------------------------------*/
long GetCEdata(int fdser, int yAddress, int opcode, int param, char description[])
{
    int nCnt;
...
int GetDSP(int fdser, int yAddress)
{
    ...
    if ((GCR = GetDSPdata(fdser,yAddress,opGetDSP,ToM2,_opGetDSP,TRUE)) < -1000000.0) return(-1);
    if ((GPR = GetDSPdata(fdser,yAddress,opGetDSP,ToM3,_opGetDSP,TRUE)) < -1000000.0) return(-1);
