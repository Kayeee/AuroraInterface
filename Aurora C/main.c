...
//CHANGE -KEVIN
BOOL bJSON = FALSE;     /* Output data in JSON*/
//END CHANGE -KEVIN
...
int main(int argc, char *argv[])
{
    ...
    if ((!GetParms(argc, argv) && !bVersion) || bHelp) {
        ...
        //CHANGE -KEVIN
        printf(" -J, --JSON                     Return data in JSON format");
        //END CHANGE -KEVIN
	...
	printf(" -U <num>, --read-pause=<num>   Pause <num> milli-seconds after sending command to inverter before\n");
        ...
        printf(" -w <num>, --lock-wait=<num>    Seconds to wait to lock serial port. (1-30)\n");
...
int GetParms(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "3a:AbC:cDd:E:efgHhi:JjK:k:L:l:M:mN:nOo:P:pQq:R:rSsTtVvU:uWw:XxY:y", longopts, NULL )) != EOF) {
        switch (c) {
            case 'a':
                /* Inverter address */
